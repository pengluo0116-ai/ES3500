...
build_apps() {
    ...
    if command -v arm-linux-gnueabihf-qmake &> /dev/null; then
        ...
        echo "  cd /root/es2000/qt_display/src && qmake && make"
    fi
...
prepare_overlay() {
    ...
    mkdir -p "$OVERLAY_DIR/root/es2000"
    mkdir -p "$OVERLAY_DIR/root/es2000/bin"
    mkdir -p "$OVERLAY_DIR/root/es2000/www"
    mkdir -p "$OVERLAY_DIR/etc/systemd/system"
    mkdir -p "$OVERLAY_DIR/etc/lighttpd/conf-enabled"

    # 复制二进制文件
    cp "$ROOT_DIR/collector/src/collector"  "$OVERLAY_DIR/root/es2000/bin/" 2>/dev/null || echo "[WARN] collector 未编译"
    cp "$ROOT_DIR/ES1000_ENYI_SERVER/web_cgi/webcgi_set/src/../bin/set.cgi" "$OVERLAY_DIR/root/es2000/bin/" 2>/dev/null || echo "[WARN] set.cgi 未编译"
    cp "$ROOT_DIR/ES1000_ENYI_SERVER/web_cgi/data_api/data_api.cgi" "$OVERLAY_DIR/root/es2000/bin/" 2>/dev/null || echo "[WARN] data_api.cgi 未编译"

    # Qt 程序
    cp "$ROOT_DIR/qt_display/src/elevator_display" "$OVERLAY_DIR/root/es2000/bin/" 2>/dev/null || echo "[WARN] elevator_display 未编译"

    # 复制 .so 库
    cp -r "$ROOT_DIR/ES1000_ENYI_SERVER/web_cgi/webcgi_set/lib/"* "$OVERLAY_DIR/usr/lib/" 2>/dev/null || true
...
create_services() {
    ...
    # 采集进程服务
    cat > "$OVERLAY_DIR/etc/systemd/system/es2000-collector.service" << 'SERVICE'
[Unit]
Description=ES2000 Elevator Data Collector
...
Restart=always
RestartSec=5
StandardOutput=append:/root/es2000/collector.log
StandardError=append:/root/es2000/collector.log
...
SERVICE
    ...
    # Qt 显示服务
    cat > "$OVERLAY_DIR/etc/systemd/system/es2000-display.service" << 'SERVICE'
[Unit]
Description=ES2000 Elevator Monitor Display
...
SERVICE
    ...
    # lighttpd 配置
    cat > "$OVERLAY_DIR/etc/lighttpd/conf-enabled/10-es2000.conf" << 'CONF'
# ES2000 CGI 配置
...
create_startup_script() {
    cat > "$OVERLAY_DIR/root/es2000/start.sh" << 'SCRIPT'
#!/bin/bash
# ES2000 系统初始化脚本
...
# 创建工作目录
mkdir -p /root/es2000/logs
mkdir -p /root/es2000/data

# 检查数据库
if [ ! -f /root/es2000/es2000.db ]; then
    echo "初始化数据库..."
    sqlite3 /root/es2000/es2000.db "VACUUM;"
fi
...
systemctl enable es2000-display
...
echo "=== ES2000 初始化完成 ==="
SCRIPT
    chmod +x "$OVERLAY_DIR/root/es2000/start.sh"
}
...
package_image() {
    ...
    OUTPUT_IMG="$OUTPUT_DIR/ES2000_firmware_$(date +%Y%m%d).img"
    ...
    # 第一次启动时运行初始化脚本
    sudo bash -c "echo '/bin/bash /root/es2000/start.sh' >> $MOUNT_DIR/etc/rc.local"
