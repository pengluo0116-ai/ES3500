...
function dev_info(){
    ...
    this.maintain = null;       //该设备上一次维保的时间
    this.jsonReal = null;       //实时数据
}
...
function create_devInfoTable(_jsonList){
    ...
    for(var i=0x00; i<num; i++){
        ...
        item_tmp.maintain       = _jsonList[i].maintain;      //上次维保时间
        item_tmp.id_parentGroup = _jsonList[i].gid;     //设备组ID
        item_tmp.mainter_time   = _jsonList[i].mainter_time;  //设备报警时间
...
function treeItem_creat(obj_lift_group){
    ...
    objstr += '<span class="folder" id="list_'+ obj_lift_group.id +'"><a href="javascript:;" onclick="">'+ obj_lift_group.name +'</a></span>';
    objstr += '<ul id="b_'+ obj_lift_group.id +'">';
    
    /*添加设备*/
    num = obj_lift_group.obj_devList.length; 
    for(var i=0x00; i<num; i++){
        objstr += '<li><span class="file" id="dev_'+ obj_lift_group.obj_devList[i].id +'"><a href="javascript:;" onclick="">'+ obj_lift_group.obj_devList[i].name +'</a></span></li>';
        // objstr += '<li><span class="file" id="dev_'+ obj_lift_group.obj_devList[i].id +'">'+ obj_lift_group.obj_devList[i].name +'</span></li>';
    }
...
/*
    函数名称：get_groupAllDev_classOBJ
    函数功能：获取指定组下所有的设备列表
    传入参数：
                class_g         组对象
                jsonList        获取到的设备存储到该列表中
    传出数据：无
    注意事项：无
    编写人员：王凤龙
    编写时间：2017-05-26
*/
function get_groupAllDev_classOBJ(class_g, jsonList){

    /*获取该组下的设备*/
    if(class_g != null){ for(var i=0x00; i<class_g.obj_devList.length; i++){ jsonList.push(class_g.obj_devList[i]); } }

    /*获取子组的设备*/
    for(var point=0x00; point<class_g.obj_childList.length; point++){ get_groupAllDev_classOBJ(class_g.obj_childList[point], jsonList); }
}

/*
    函数名称：get_groupAllDev_classOBJ_list
    函数功能：获取所有的设备列表--根据分组设置前后顺序
    传入参数：jsonList   获取到的设备存储到该列表中
    传出数据：无
    注意事项：无
    编写人员：王凤龙
    编写时间：2017-05-26
*/
function get_groupAllDev_classOBJ_list(jsonList){
    for(var point=0x00; point<lift_group_LIST.obj_childList.length; point++){ 
        get_groupAllDev_classOBJ(lift_group_LIST.obj_childList[point], jsonList);
    }
}
...
/*
    ...
    编写时间：2017-05-26
*/
function get_dev_gname(dev_classOBJ){
    var name = "";
    var obj_g = null;
    var obj_gg = null;
    
    obj_g = dev_classOBJ.obj_parentGroup;
    obj_gg = obj_g.obj_parent;

    if(obj_gg != null) name += obj_gg.name + "/";
    if(obj_g != null) name += obj_g.name + "/";
    name += dev_classOBJ.name;
...
var test_devlist = [
    ...
    {"id":"5002", "gid":2, "name":"11设备"},
    {"id":"5002", "gid":2, "name":"11设备"},
    {"id":"5003", "gid":3, "name":"12设备"},
    {"id":"5004", "gid":4, "name":"13设备"},
    {"id":"5004", "gid":4, "name":"13设备"},
    {"id":"5004", "gid":4, "name":"13设备"},
    {"id":"5004", "gid":4, "name":"13设备"},
    {"id":"5004", "gid":4, "name":"13设备"},
