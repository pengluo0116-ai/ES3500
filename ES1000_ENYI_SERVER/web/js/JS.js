...
function isMAC(mac){
    ...
    if (!temp.test(mac)) return false; else return true;
}

/*
    函数名称：get_timeDifference
    函数功能：计算时间戳
    传入参数：_newDt, _oldDt
    传出数据：时间差
    注意事项：无
    ...
    编写时间：2016-10-21
*/
function get_timeDifference(_newDt, _oldDt){
    var newDt; 
    var oldDt;
    try{
        newDt = new Date(_newDt);
        oldDt = new Date(_oldDt);
    }catch(e){ return 0x00; }
    
    return newDt.getTime() - oldDt.getTime();
}
