...
int usart_read::sopen(){
    if((this->fd = open(this->ttl_path.c_str(), O_RDWR | O_NOCTTY)) <= 0x00) return -0x06;
...
/*
    函数名称:writeData
    函数功能:向串口写数据
    传入参数:dataBuff要写入的数据,len写入的长度
    传出数据:0代表成功,非0代表失败
    编写人员:王清杰
    编写时间:2018-3-1
*/
int usart_read::writeData(unsigned char *dataBuff, int len)
{
    return write(this->fd,dataBuff,len);
}