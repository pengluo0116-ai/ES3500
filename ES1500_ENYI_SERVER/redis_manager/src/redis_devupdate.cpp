...
static int dev_create_info(Json::Value &jsonList_dev, Json::Value &jsonList_group){
    ...
    for(unsigned int point=0; point<jsonList_dev.size(); point++){
        if(group_getJsonByID(jsonList_dev[point]["gid"].asInt(), jsonList_group, jsonG)){
            ...
            jsonList_dev[point]["pgid"] = jsonG["gid"].asInt();
            if(jsonG["gid"].asInt() == 0x00) jsonList_dev[point]["pgname"] = "";
            else jsonList_dev[point]["pgname"] = group_getNameByID(jsonG["gid"].asInt(), jsonList_group);
        }
        else{
...
/*
    函数名称：cmd_device_redisUpdate
    函数功能：将设备信息更新到REDIS
    传入参数：Json::Value &jsonList_Info 设备属性信息列表
    传出数据：
                0  运行成功
                1  redis开启失败
                2  向redis更新数据失败
    注意事项：设备属性信息中，已经包含了该设备的组信息
    编写人员：王凤龙
    编写时间：2017-06-06
*/
int cmd_device_redisUpdate(Json::Value &jsonList_Info){
    return redis_devUpdate(jsonList_Info);
}
