...
/*
    ...
    编写时间：2016-12-07
*/
net_cont::net_cont(const char *_ip, const char *_gateway, const char *_netmask, const char *_mac):
ip(string(_ip)), gateway(string(_gateway)), netmask(string(_netmask)), mac(string(_mac)){}
...
/*
    ...
    编写时间：2017-03-10
*/
int net_cont::_ck_mac(){
    /*参数校验*/
    if(this->mac.size() != 0x11) return -11;

    /*MAC校验*/
    unsigned int _mac_[6] = {0x00};
    if(
        sscanf(this->mac.c_str(), "%02x:%02x:%02x:%02x:%02x:%02x", &_mac_[0], &_mac_[1], &_mac_[2], &_mac_[3], &_mac_[4], &_mac_[5]) != 0x06 &&
        sscanf(this->mac.c_str(), "%02X:%02X:%02X:%02X:%02X:%02X", &_mac_[0], &_mac_[1], &_mac_[2], &_mac_[3], &_mac_[4], &_mac_[5]) != 0x06             
    ) return -12;
...
int net_cont::check(){
    ...
    /*校验mac地址*/
    if(err = this->_ck_mac()) return err;
    return err;
}
