...
void getPostData(){
    ...
    len = atoi(lenStr);
    if((len > (GET_DATA_BUFF_LEN-1)) || (len <= 0x00)) { err_msg("上传数据错误"); return;} //上传数据量超限，返回错误信息
    ...
    try{
        ...
        switch(display){
            /*分支操作，添加此处*/
            /*==========================================*/
            case 1: cmd_getConf(); break;
            //case 2: cmd_setConf(value); break;
            case 3: cmd_setTime(value); break;
            case 4: cmd_getTime(); break;
            /*==========================================*/
            default: err_msg("上传数据格式错误1"); break;
        }
    }catch(...){
        err_msg("上传数据格式错误2");
    }
